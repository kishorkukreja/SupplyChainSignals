...
**CLI path: `/root/.local/bin/hyperresearch`** — use this exact path for every hyperresearch command. It may not be on your system PATH.
...
```bash
/root/.local/bin/hyperresearch run status -j                 # Newest run: step status, spend, escalation queue depth
/root/.local/bin/hyperresearch run resume -j                 # Exact next step + Skill invocation to continue with
/root/.local/bin/hyperresearch run report -j                 # Per-step wall-time / spend / event telemetry
/root/.local/bin/hyperresearch run verify <vault_tag> -j     # Ship gate: headings, length, citation density, cite-check resolution
```

Blocked fetches (login walls, bot walls, captchas) queue as escalations instead of dying: `/root/.local/bin/hyperresearch escalation list --status queued -j`. The browser-fetcher agent drains them via the user's real Chrome; CAPTCHAs / logins / 2FA are ALWAYS handed to the human, consolidated into one message.
...
```bash
/root/.local/bin/hyperresearch scholar search "<query>" --limit 25 -j          # every available source, deduplicated
/root/.local/bin/hyperresearch scholar search "<query>" --scope papers -j      # literature only, no trials/filings/series
/root/.local/bin/hyperresearch scholar search "<query>" -s openalex -s core -j # pick specific sources
/root/.local/bin/hyperresearch scholar sources -j                              # what is available, what each covers
```
...
`/root/.local/bin/hyperresearch fetch` auto-detects PDF URLs (arXiv, NBER, SSRN, direct `.pdf` links) and extracts full text via pymupdf. Fetch them aggressively. Raw PDFs land in `research/raw/<note-id>.pdf` and the note's frontmatter links back via `raw_file:`.
...
- `/root/.local/bin/hyperresearch note show <id> -j` carries an `oa` block with `body_is_not_from_source: true`,
...
```bash
/root/.local/bin/hyperresearch search "query" --json                # Full-text search
/root/.local/bin/hyperresearch search "query" --tag ml --json       # Filter by tag / status / date / parent
/root/.local/bin/hyperresearch search "query" --include-body --json # Full-body search, not just titles
/root/.local/bin/hyperresearch note show <id> --json                # Read one note
/root/.local/bin/hyperresearch note show <id1> <id2> <id3> --json   # Batch-read notes in one call
/root/.local/bin/hyperresearch note list --json                     # List all notes with summaries
/root/.local/bin/hyperresearch tags --json                          # Existing tag vocabulary
```
...
`/root/.local/bin/hyperresearch note show <id>` (single, batch, or `-j`) or via `/root/.local/bin/hyperresearch search`
...
```bash
/root/.local/bin/hyperresearch fetch "<url>" --tag <topic> --save-assets -j   # Saves screenshot + top images
/root/.local/bin/hyperresearch assets list --note <note-id> --json            # Assets for a specific note
/root/.local/bin/hyperresearch assets path <note-id> --type screenshot -j     # Get screenshot path (viewable with Read)
```

### Authenticated crawling

Login-gated content (LinkedIn, Twitter, paywalled news) needs a browser profile. Set up once via `/root/.local/bin/hyperresearch setup` or `crwl profiles`. Config in `.hyperresearch/config.toml` under `[web]`: `profile = "research"`, `magic = true`. LinkedIn / Twitter / Facebook / Instagram / TikTok auto-use a visible browser to avoid session kills.

If a fetch returns a login wall, tell the user to run `/root/.local/bin/hyperresearch setup` and create a login profile.
...
```bash
/root/.local/bin/hyperresearch note list --status draft -j                                        # Find unprocessed notes
/root/.local/bin/hyperresearch note show <id> -j                                                  # Read the content
/root/.local/bin/hyperresearch note update <id> --summary "<specific summary>" --add-tag <t> -j   # Add summary + tags
/root/.local/bin/hyperresearch lint -j                                                            # Find missing tags / summaries / broken links
/root/.local/bin/hyperresearch repair -j                                                          # Auto-fix broken links, rebuild indexes
/root/.local/bin/hyperresearch sources score -j                                                   # Enrich DOI-bearing sources (citations, venue, retractions) + recompute quality
/root/.local/bin/hyperresearch graph rank -j                                                      # Recompute vault PageRank centrality
/root/.local/bin/hyperresearch status -j                                                          # Overall vault health
```

Lifecycle: `draft` → `review` → `evergreen` (or `stale` → `deprecated` → `archive` for outdated material).

Summaries must be specific — "Mamba achieves linear-time sequence modeling via selective state spaces" beats "Paper about Mamba". Reuse the existing tag vocabulary (`/root/.local/bin/hyperresearch tags -j`) rather than inventing new tags.

### Key conventions

- Notes live in `research/notes/` as markdown with YAML frontmatter
- Link notes with `[[note-id]]` syntax
- After editing `.md` files directly, run `/root/.local/bin/hyperresearch sync` to update the index
- Run `/root/.local/bin/hyperresearch --help` for the full command list
<!-- hyperresearch:end -->
