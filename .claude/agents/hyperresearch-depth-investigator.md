...
1. **Start with the vault.** Before fetching anything new, read the notes
   ...
   `/root/.local/bin/hyperresearch note show <id1> <id2> <id3> --json`
...
3. **Fetch new sources via the fetcher subagent.** Do NOT call
   `/root/.local/bin/hyperresearch fetch` directly. Delegate to `hyperresearch-fetcher` via the
...
5. **Read the fetched sources.** Use `/root/.local/bin/hyperresearch note show <id> -j`. Quote
...
6. **Write ONE interim report note.** This is your single deliverable.
   ...
   **BEFORE calling `note new`**, check if an interim note for this locus
   ...
   ```bash
   /root/.local/bin/hyperresearch note list --tag locus-<locus-name> --type interim --all --json
   ```
...
```bash
/root/.local/bin/hyperresearch note new "Interim report — <locus name>" \
  --tag <corpus_tag> \
