...
2. **Verify positions against original sources.** For each committed
   ...
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch note show <id1> <id2> <id3> -j
   ```
...
5. **Search the vault** for existing sources that might already contain
   ...
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch search "<adversarial query>" --tag <corpus_tag> -j
   ```
