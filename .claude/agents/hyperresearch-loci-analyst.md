...
1. **Load the corpus.** Use `/root/.local/bin/hyperresearch note list --tag <corpus_tag> --all --json`
...
2. **Read breadth first.** For each note, read the title + summary + first
   ~400 chars (use `/root/.local/bin/hyperresearch note show <id> -j` and truncate). Do NOT read
