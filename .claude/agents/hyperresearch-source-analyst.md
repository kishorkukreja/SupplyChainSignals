...
- **source_note_id**: the vault note id of the source you will analyze
  ...
  will call `/root/.local/bin/hyperresearch note show <source_note_id> -j` to read the
  full body.
...
1. **Check for an existing analysis.** Before writing anything, search:
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch note list --tag <vault_tag> --type source-analysis --all --json
   ```
...
2. **Read the source.** Pull the full body:
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch note show <source_note_id> -j
   ```
...
5. **Create the source-analysis note:**
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch note new "Source Analysis — <short title>" \
