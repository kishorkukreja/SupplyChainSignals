...
```bash
PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch fetch "<url>" --tag <topic> -j
```
...
```bash
PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch fetch "<url>" --tag <topic> --utility-score 14 -j
```
...
```bash
PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch fetch "<url>" \
  --tag <topic> \
...
1. Check if it's already fetched:
   `PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch sources check "<url>" -j`

2. If not already fetched, fetch it:
   `PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch fetch "<url>" --tag <topic> -j`

3. After fetching, read the note content:
   `PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch note show <note-id> -j`
...
4. **Quality check** — read the content and decide:
   - Is this actually relevant to the research topic? If completely off-topic, deprecate it:
     `PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch note update <note-id> --status deprecated -j`
...
5. If the content is good, write a real summary and add tags:
   `PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch note update <note-id> --summary "<specific summary>" -j`
   `PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch note update <note-id> --add-tag <specific-tag> -j`
...
2. **Find and fetch the primary sources.** For each priority lead:
   - If you have a direct URL from the citation, fetch it with the
     ...
     ```
     PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch sources check "<url>" -j
     PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch fetch "<url>" --tag <topic> --suggested-by <note-id-that-cited-it> --suggested-by-reason "cited as primary source" -j
     ```
   ...
   - Once you have the URL, fetch it with `/root/.local/bin/hyperresearch fetch` as above.
...
3. **Process each discovered source** with the same full procedure as
   Phase 1: read the note content with `/root/.local/bin/hyperresearch note show <id> -j`,
   quality check, write summary with `/root/.local/bin/hyperresearch note update`, add tags,
