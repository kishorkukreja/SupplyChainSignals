...
```bash
PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch escalation human <id> --detail "<one line: site + what the human must do>" -j
```
...
1. **Claim:**
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch escalation claim --by browser-fetcher --tag <vault_tag> -j
   ```
...
4. **Ingest.** Write the extracted content to a scratch file, then:
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch escalation ingest <id> --title "<page title>" --body-file <scratch-file> --tag <topic-tag> -j
   ```
...
5. **Genuinely unreachable** (dead page, geo-block, content gone):
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch escalation abandon <id> --detail "<why>" -j
   ```
...
4. For the 2-3 highest-citation results directly relevant to the research
   ...
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch escalation add "<paper url>" --reason interactive_needed --tag <vault_tag> --suggested-by <scholar-note-id> --detail "high-citation Scholar hit" -j
   ```
