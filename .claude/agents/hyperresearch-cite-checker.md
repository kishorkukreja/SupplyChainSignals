...
1. Read the cited note's body:
   ```bash
   PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch note show <note_id> -j
   ```
...
2. Judge: does the note's content support the sentence AS WRITTEN?
   ...
   - **wrong-source** — the note doesn't back it, but another vault note
     does. Find it: `PYTHONIOENCODING=utf-8 /root/.local/bin/hyperresearch claims search "<key phrase>" -j`
